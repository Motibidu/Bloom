...
description: >
  ...
  for solo developers or small projects. This agent specializes in generating
  practical, development-ready specifications without corporate complexity.
  Use it when: starting a new project and need clear requirements, converting
  vague ideas into actionable development plans, or documenting features for
...
- AWS EC2 - 어플리케이션 서버 호스팅 (Docker 컨테이너 구동)
- AWS S3 - 정적 자원(이미지, 파일) 저장소
- AWS RDS (MySQL) - 데이터베이스 호스팅 및 자동 백업 관리
- Route 53 - DNS 관리 및 SSL 인증서 적용
- Nginx - 리버스 프록시(Reverse Proxy) 및 부하 분산, 정적 파일 서빙
- Certbot (Let's Encrypt) - SSL/TLS 인증서 자동 발급 및 HTTPS 보안 연결 적용

### 🐳 컨테이너화 & 오케스트레이션
Docker - 어플리케이션의 환경 독립성 확보 및 이미지 빌드
Docker Compose - 앱 서버(Spring Boot)와 DB 등 다중 컨테이너 통합 관리

### 🔄 CI/CD 파이프라인
Jenkins - 설치 방식: EC2 내부에서 Docker 이미지로 구동
GitHub Webhooks - 코드 Push 시 Jenkins 빌드 자동 트리거
Pipeline-as-Code - Jenkinsfile을 통한 배포 스크립트 버전 관리

### 📄 단일 서버 배포 아키텍처
1. 사용자 요청: 도메인 접속 (HTTPS 443) → Nginx 컨테이너가 수신

2. 리버스 프록시:
- 일반 접속 (/) → App 컨테이너 (8080)로 전달
- 관리 접속 (/jenkins) → Jenkins 컨테이너 (8080)로 전달 (IP 화이트리스트 권장)

3. CI/CD 흐름:
- GitHub Push → Jenkins 컨테이너가 감지
- Jenkins 내부에서 소스 빌드 및 Docker 이미지 생성
- docker-compose up -d 명령을 통해 호스트 서버의 App 컨테이너를 새 버전으로 교체
...
- **최신 버전 필수**: React 19, TailwindCSS v4 등 최신 버전 사용
